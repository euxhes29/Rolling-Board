...
const EditProfilePopup = ({ isProfileMenuOpen, handleCloseProfileClick }) => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (response.data) {
          ...
          console.log("ID e ruajtur në Zustand:", useUserStore.getState().id);
        }
      } catch (error) {
        console.error(
  ...
  const handleEditProfileClick = () => {
    console.log("Edit Profile button clicked");  // Debugging message
    setIsEditProfilePopupOpen(true);
  };

  const handleCloseEditProfileClick = () => {
    console.log("Popup closed");  // Debugging message
    ...
    <>
      ...
      {isEditProfilePopupOpen && (
        <Popup
          ...
          btnName2="Cancel"
        >
          <FormProvider {...{ handleSubmit, register, errors }}> {/* Wrapping with FormProvider */}
            <InputField
              label="First Name"
              type="text"
              name="firstname"
